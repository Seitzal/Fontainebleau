...
object FunCSV {
  ...
  /**
    ...
    * @param always_quote Whether to  surround all elements in quotation marks,
    *                     not just those containing commas or quotation marks.
    * @return The string containing the serialised data
    */
  def encode(data : List[List[String]],
      always_quote : Boolean = false) : String = {
    ...
    @tailrec def iter(remainder : List[List[String]],
  ...
  /**
    * Replaces Windows-style and old Macintosh-style line endings with Linux
    * style line endings.
    */
  ...
  /**
    ...
    * @return The two-dimensional immutable list containing the deserialised
    *         data
    */
  def decode(raw : String) : List[List[String]] = {
    ...
    def parseLine(line : String) : List[String] = {
      ...
      val escapedCommas : List[Int] = {
        @tailrec def iter(i : Int, inQuotedBlock : Boolean,
            current : Vector[Int]) : Vector[Int] = {
          if(i < line.length) {
      ...
      @tailrec def split(i : Int, elementStart : Int,
          current : Vector[String]) : Vector[String] = {
        if(i < line.length) {
          val char = line.charAt(i)
      ...
      def cleanup(lines : List[String]) : List[String] = {
        def cleanFirst(x : String) =
          if(x.charAt(0) == '\"')
            x.substring(1)
          else x
        def cleanLast(x : String) =
  ...
  /**
    * Serialises the specified data using the CSV format, and saves it as a file
    * @param data A two-dimensional list containing the data
    * @param path The path to the file where the data should be saved.
    *             The file will be created if it is missing, and overwritten if
    *             it exists
    * @param always_quote Whether to  surround all elements in quotation marks,
    *                     not just those containing commas or quotation marks
    * @return The string containing the serialised data
    */
  def encodeFile(data : List[List[String]], path : String,
      always_quote : Boolean = false) : String = {
